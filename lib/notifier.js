...
Notifier.prototype = {
  ...
  send: function (action, notification, cb) {
    var config = this.config,
        // Simple async.parallel implementation
        callback = function () {
          callback.counted = (callback.counted || 0) + 1;
          if (callback.counted === callback.count) {
            return cb && cb();
          }
        };
    ...
    if (config.APN) {
      callback.count++;
      this.APN(notification, callback);
    }
    ...
    if (config.email) {
      ...
      callback.count++;
      this.mail(notification, template, callback);
    }
