...
var Notifier = function (config) {
  if (config) {
    ...
    if (!this.config.tplType) this.config.tplType = 'pug';
 ...
 * templating language. Default is pug
 *
...
Notifier.prototype.processTemplate = function (tplPath, locals) {
  ...
  var html = Pug.compile(tpl, { filename: tplPath });
